    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_CC_DIR/.claude/hooks/session-register.sh",
            "timeout": 10
          }
        ]
      }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_CC_DIR/.claude/hooks/tldr-read-enforcer.sh",
            "timeout": 20
          }
        ]
      }
...
- **command**: Path to hook script (use `$CLAUDE_CC_DIR` or `$HOME` for portability)
...
- Use `$CLAUDE_CC_DIR` paths to scope to current project
