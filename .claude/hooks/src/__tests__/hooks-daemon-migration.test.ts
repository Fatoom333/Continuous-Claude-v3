...
function setupTestEnv(): void {
  ...
  process.env.CLAUDE_CC_DIR = TEST_PROJECT_DIR;
}
...
describe("Common daemon migration patterns", () => {
  ...
  describe("project directory resolution", () => {
    it("should use CLAUDE_CC_DIR when available", () => {
      process.env.CLAUDE_CC_DIR = "/custom/project";
    ...
    it("should fall back to cwd when env not set", () => {
      ...
      const getProjectDir = (): string => {
        return process.env.CLAUDE_CC_DIR || "/fallback/dir";
      };
